...
void send_to_one(int sock, const char *msg) {
...
void run_lobby(int server_fd, int *clients, Player *players, int *count) {
    ...
    while (*count < MAX_PLAYERS) {
        ...
        if (bytes <= 0) { close(new_fd); continue; }
...
void build_state_msg(GameState *gs, char *buf, int buf_size) {
    ...
    for (int i = 0; i < gs->player_count; i++) {
        char part[32];
        snprintf(part, sizeof(part), "%s=%d ", gs->players[i].name, gs->players[i].score);
...
int run_round(GameState *gs, int *clients, int count) {
    ...
    while (!is_game_over(gs)) {
        ...
        for (int i = 0; i < count; i++) {
            if (i != cur) send_to_one(clients[i], wait_msg);
        }
        ...
        if (result == 1) {
            ...
            snprintf(result_msg, sizeof(result_msg), "MISS! '%c' is not in the word.\n", guess);
        } else {
            snprintf(result_msg, sizeof(result_msg), "Already tried '%c'.\n", guess);
        }
        send_to_all(clients, count, result_msg);

        if (!is_game_over(gs)) next_player(gs);
    }
...
int main() {
    ...
    if (word_count <= 0) { fprintf(stderr, "Could not load words.txt\n"); return 1; }
